...
function RoleRouter() {
  const { loading, contextError, bootstrapMessage, primarySurface, roles, refresh, isAuthenticated } = useAuth();
  const navigate = useNavigate();

  useEffect(() => {
    if (loading || contextError || bootstrapMessage) return;
    ...
    <div className="flex min-h-screen items-center justify-center bg-background">
      <div className="text-center">
        ...
        <p className="mt-6 text-sm text-muted-foreground">
          {loading ? "Loading your workspace…" : contextError ? "We couldn't load your Studio access." : bootstrapMessage ?? (roles.length === 0 ? "Setting up your account…" : "Redirecting…")}
        </p>
        ...
        {!loading && !bootstrapMessage && roles.length === 0 && isAuthenticated && (
